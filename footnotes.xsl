...
<xsl:param name="next-note-list"></xsl:param>
...
<xsl:template match="note">
  <xsl:param name="mode">normal</xsl:param>	<!-- also possible: next-note, ref -->
  ...
  <xsl:choose>
    <xsl:when test="$mode = 'ref' and $ref-mode = 'ref' and $use-popups = 'true'">
      <xsl:attribute name="onMouseOver"> 
        <xsl:text>overlib(</xsl:text>
    <xsl:call-template name="id-to-jsvar">
      <xsl:with-param name="id"><xsl:value-of select="@id"/></xsl:with-param>
    </xsl:call-template>
    <xsl:text>, </xsl:text><xsl:if test="$popup-captions = 'true'"><xsl:text>CAPTION, 'Footnote </xsl:text><xsl:value-of select="$number"/><xsl:text>', </xsl:text></xsl:if><xsl:text>WIDTH, </xsl:text><xsl:value-of select="$popup-width"/><xsl:value-of select="$cssopts"/><xsl:value-of select="$STICKY"/><xsl:text>);</xsl:text></xsl:attribute> 
...
<xsl:text>, </xsl:text><xsl:if test="$popup-captions = 'true'"><xsl:text>CAPTION, 'Footnote', '</xsl:text><xsl:value-of select="$number"/></xsl:if><xsl:text>', WIDTH, </xsl:text><xsl:value-of select="$popup-width"/><xsl:value-of select="$cssopts"/><xsl:value-of select="$STICKY"/><xsl:text>);</xsl:text> 
        </xsl:attribute>
        <xsl:attribute name="onMouseOut">
          <xsl:text>nd();</xsl:text>
        </xsl:attribute>
     ...
     <xsl:element name="div">
       ...
       <a href="http://www.bosrup.com/web/overlib/">
         <xsl:element name="img">
           <xsl:attribute name="src"><xsl:value-of select="$overlib-graphic"/></xsl:attribute>
           <xsl:attribute name="width">88</xsl:attribute>
           <xsl:attribute name="height">31</xsl:attribute>
           <xsl:attribute name="alt">Popups by overLIB</xsl:attribute>
...
<!-- convert id to a string safe to use for a javascript variable name -->
<xsl:template name="id-to-jsvar">
  <xsl:param name="id"/>

  <xsl:call-template name="replace-string">
    <xsl:with-param name="string"><xsl:value-of select="$id"/></xsl:with-param>		
    <xsl:with-param name="from"><xsl:text>.</xsl:text></xsl:with-param>
    <xsl:with-param name="to"><xsl:text>_</xsl:text></xsl:with-param>
  </xsl:call-template>
</xsl:template>
